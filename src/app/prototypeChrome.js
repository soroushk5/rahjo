...
const destinations = Object.freeze([
  ...
  { path: "/dashboard", label: "کنسول داده", meta: "درخواست‌ها و آمادگی" },
  { path: "/request", label: "درخواست دسترسی", meta: "فرایند نمونه" }
]);
    ...
    <section class="command-dialog" role="dialog" aria-modal="true" aria-labelledby="command-title">
      <header>
        <div>
          ...
          <h2 id="command-title">کجا می‌خواهید بروید؟</h2>
        </div>
        <button type="button" class="icon-button" data-command-close aria-label="بستن">${icon("close")}</button>
      </header>
      <label class="command-search">
        ${icon("search")}
        <input id="command-query" autocomplete="off" placeholder="جست‌وجوی صفحه یا مفهوم…" />
      </label>
      <nav class="command-results" aria-label="نتایج جست‌وجوی سریع">
        ${destinations.map((item) => `
          <a data-link data-command-item data-command-text="${item.label} ${item.meta}" href="${item.path}">
            <strong>${item.label}</strong>
            <small>${item.meta}</small>
            <span>${icon("arrow")}</span>
          </a>`).join("")}
      </nav>
      <footer><kbd>Esc</kbd> بستن · <kbd>Ctrl K</kbd> باز کردن</footer>
    </section>`;
}
...
function mountCommandPalette() {
  const palette = ensureCommandPalette();
  document.querySelectorAll("#global-search,[data-open-command]").forEach((button) => {
