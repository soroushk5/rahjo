...
export class Router {
  ...
  constructor({ root, routes }) { this.root = root; this.routes = routes; this.handleNavigation = this.handleNavigation.bind(this); }
  start() {
    ...
    document.addEventListener("click", (event) => {
      ...
      event.preventDefault(); this.navigate(target.pathname);
    });
    this.handleNavigation();
  }
  /** @param {string} path */ navigate(path) { window.history.pushState({}, "", path); this.handleNavigation(); }
  handleNavigation() {
    ...
    this.root.innerHTML = route.render(); route.mount?.(); window.scrollTo({ top: 0, behavior: "instant" });
  }
}
