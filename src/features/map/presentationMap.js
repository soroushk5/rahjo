...
function detailPanel() {
  ...
  return `<div class="map-detail__heading"><span>${icon(meta.icon, { size: 23 })}</span><div><small>${meta.label}</small><h2>${node.label}</h2></div></div><p>${node.description}</p><dl><div><dt>ورودی از</dt><dd>${incoming.length ? incoming.join("، ") : "—"}</dd></div><div><dt>خروجی به</dt><dd>${outgoing.length ? outgoing.join("، ") : "—"}</dd></div></dl><div class="map-detail__explain">${icon("node", { size: 18 })}<span>با انتخاب هر گره، مسیرهای مستقیم مرتبط برجسته می‌شوند.</span></div>`;
}
